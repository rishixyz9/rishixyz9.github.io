...
export default function Card(props: Props) {
    ...
    <div className='min-h-[480px] h-full flex flex-col relative md:w-[360px] self-center backdrop-blur-[2.5px] p-4 rounded-lg border-h-purple border'>
        <Image 
            className={`z-0 top-0 h-[240px] w-full self-center justify-center object-cover ${props.data.imgPos ? props.data.imgPos : "object-center"} rounded-md opacity bg-white`}
        ...
        {props.data.desc.map((item, key) => 
            <div className='text-left font-light text-sm' key={key}>{item}</div>
        )} 
